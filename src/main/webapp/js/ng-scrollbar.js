...
myapp.controller('myctrl1', function ($scope) {
    ...
    $scope.getCollapse = function(){
        //alert("hehe");
        return ($scope.isCollapse?"navbar-cart collapse in":"navbar-cart collapse")
    };
  ...
  function ($parse, $window) {
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        var _transculdedContainer = new Object();
        var redraw = function () {
          ...
          transculdedContainer.css('top', page.top + 'px');
            console.log("in redraw, page top: " + page.top + " page.scrollHeight: " + page.scrollHeight);
        };
        ...
        var wheelHandler = function (event) {
          ...
          if(page.scrollHeight != scope.elemobj.scrollHeight){
              buildScrollbar();
          }
          var wheelDivider = 20;
          ...
          dragger.top = Math.max(0, Math.min(parseInt(page.height, 10) - parseInt(dragger.height, 10), parseInt(dragger.top, 10) - deltaY));
            console.log("page top: " + page.top + " page.height: " + page.height + " dragger.height: " + dragger.height);
          redraw();
        ...
        var _touchDragHandler = function (event) {
            console.log("Touch drag handler" );
          var newOffsetX = 0;
        ...
        var buildScrollbar = function (rollToBottom) {
          ...
          _transculdedContainer = transculdedContainer;
          scope.elemobj = transculdedContainer[0];

            console.log("page height: " + page.height + " page.scroll: " + page.scrollHeight);
          scope.watch('elemobj.scrollHeight', function(){
            console.log("change scroll height");
          });
          if (page.height < page.scrollHeight) {
            ...
            if (rollToBottom) {
              ...
              dragger.top = parseInt(page.height, 10) - parseInt(dragger.height, 10) ;
                ...
                console.log("dragger height: " + dragger.height);
                console.log("dragger top: " + dragger.top);
            } else {
      ...
      template: '<div>' + '<div class="ngsb-wrap">' + '<div class="ngsb-container" ng-transclude></div>'
          + '<div class="ngsb-scrollbar" style="position: absolute; display: block;" ng-show="showYScrollbar">'
          + '<div class="ngsb-thumb-container">'
          + '<div class="ngsb-thumb-pos" oncontextmenu="return false;">' + '<div class="ngsb-thumb" ></div>'
          + '</div>' + '<div class="ngsb-track"></div>' + '</div>' + '</div>' + '</div>' + '</div>'
    };
  }
]);